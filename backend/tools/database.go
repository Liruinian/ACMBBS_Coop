...
type DetailedUserInfo struct {
	...
	UDescription  string `gorm:"column:uDescription" json:"uDescription"`
	UGender       string `gorm:"column:uGender" json:"uGender"`
	UIntroduction string `gorm:"column:uIntroduction" json:"uIntroduction"`
	ULikedComment string `gorm:"column:uLikedComment" json:"uLikedComment"`
	UProfession   string `gorm:"column:uProfession" json:"uProfession"`
}
...
func GetFromUsername(db *gorm.DB, username string) (results []*Login, err error) {
	err = db.Table("login").Model(Login{}).Where("`username` = ?", username).Find(&results).Error
	return
}
